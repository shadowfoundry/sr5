...
export default async function matrixActionInfo(cardData, actorId){
	...
	if (cardData.test.typeSub === "matrixSearch"){
		...
		return cardData.title = `${game.i18n.localize("SR5.MatrixActionTest")}${game.i18n.localize("SR5.Colons")} ${game.i18n.localize(SR5.matrixRolledActions[cardData.test.typeSub])} (${cardData.threshold})`;
	}
	...
	if (cardData.roll.hits > 0) {
		...
		else if (cardData.test.typeSub === "eraseMark") {
			if (cardData.target.actorId) cardData.chatCard.buttons.eraseMark = SR5_RollMessage.generateChatButton("nonOpposedTest", "eraseMark", game.i18n.localize("SR5.ChooseMarkToErase"));
			else cardData.chatCard.buttons.eraseMark = SR5_RollMessage.generateChatButton("opposedTest", "eraseMark", game.i18n.localize("SR5.ChooseMarkToErase"));
		}
