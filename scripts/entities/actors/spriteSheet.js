...
export class SR5SpriteSheet extends ActorSheetSR5 {
	...
	_prepareMatrixActions(actor) {
		...
		for (let [key, matrixAction] of Object.entries(actor.system.matrix.actions)) {
			if (matrixAction.test?.dicePool > 0 || matrixAction.defense?.dicePool > 0 || (killCodeRules && matrixAction.source === "killCode") || this._shownNonRollableMatrixActions) activeMatrixActions[key] = matrixAction;
		}
